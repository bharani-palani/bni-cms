...
function UploadDropZone(props) {
  ...
  useEffect(() => {
    if (Object.keys(progress).length > 0) {
      ...
      let files = bprogFiles
        .reverse()
        .filter(
          (v, i, a) =>
            a.findIndex(t => t.Key === v.Key) === i && v.total !== v.loaded
        );
      ...
      >
        {({ getRootProps, getInputProps, isDragAccept, isDragReject }) => {
          ...
          if (isDragReject) {
            ...
            placeholder = (
              <div className="upload-error">File type not allowed</div>
            );
          }
          if (!isDirectory) {
            ...
            placeholder = (
              <div>
                <i className="fa fa-folder-open" /> Select a folder to upload
                files
              </div>
            );
          }
